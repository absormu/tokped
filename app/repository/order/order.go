...
func GetOrderHistory2(c echo.Context, orderID string) (order entity.OrderHistory, e error) {
	...
	query := "SELECT o.id, o.created_at, o.seller_id, o.seller_name, o.buyer_id, o.buyer_name, " +
		...
		"od.product_name, od.product_weight, od.product_price, od.quantity, od.total_amount " +
		"FROM orders o " +
	...
	for result.Next() {
		if e = result.Scan(
			...
			&order.Total.ServiceCharge, &order.Total.Amount,
			&order.OrderDetails.ProductName, &order.OrderDetails.ProductWeight, &order.OrderDetails.ProductPrice, &order.OrderDetails.Quantity, &order.OrderDetails.TotalAmount); e != nil {
			return
		}
	}
...
func GetOrderHistory(c echo.Context, orderID string) (orders []entity.OrderHistory, e error) {
	logger := md.GetLogger(c)
	logger.WithFields(logrus.Fields{
		"params": orderID,
	}).Info("repository: GetOrderHistory")
